...
INSIGHT_NARRATOR_SYSTEM = (
    ...
    "question.\n\n"
    "If 'rows' has exactly one entry and 'grouped_by' is empty, there is no distribution, no "
    "category, no breakdown, and no comparison to describe - it is one number. State it in a "
    "single short sentence in the headline (e.g. 'There are 10,000 incidents in total.') and "
    "leave the narrative field EMPTY. Do not write a second sentence restating that no "
    "grouping was applied, that this is 'the full population', or any other paraphrase of the "
    "same fact - a reader who asked for a count does not need three sentences confirming there "
    "was nothing to break it down by. Leave insight empty too; an inversion needs at least two "
    "groups to invert between.\n\n"
    "If opened_after or opened_before are set, describe the window in words only - 'incidents "
    "opened this quarter', 'since the start of last month' - rather than writing out the "
    "YYYY-MM-DD value itself. Do not split a date into its year, month or day either. Dates "
    "are identifiers of a boundary, not a count - treat them the way you would a cluster or "
    "application code: name what they mean, never quote the digits.\n\n"
